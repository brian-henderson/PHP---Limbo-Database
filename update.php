...
<div id = "header">
    ...
    <div id = "message">
        <div id = "messageT1"> Update Items </div>
        <div id = "messageT2"> Update and change item fields here!</div>
        <br>
    </div>
...
#Change Item Status by the passed in Item ID number
    if ($_SERVER[ 'REQUEST_METHOD' ] == 'POST') {
	   ...
	   $updateItem = 'UPDATE stuff SET item_name = "'.$item_name.'", description = "'.$description.'", location_id = "'.$location_id.'", create_date = "'.$create_date.'", update_date= "'.$update_date.'", owner = "'.$owner.'", finder = "'.$finder.'", status = "'.$status.'" WHERE id = "'.$id.'"';
...
?>    
    ...
    <table>
        ...
        <tr><td>Location:</td><td><select name = "location_id" value="<?php if (isset($_POST['location_id'])) echo $_POST['location_id'];?>">
         ...
         <tr><td>Create Date:</td><td><input type="datetime-local" name="create_date" value="<?php if (isset($_POST['create_date'])) echo $_POST['create_date'];?>"></td></tr>
         
		 <tr><td>Update Date:</td><td><input type="datetime-local" name="update_date" value="<?php if (isset($_POST['update_date'])) echo $_POST['update_date'];?>"></td></tr>
